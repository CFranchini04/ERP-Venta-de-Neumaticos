import facturasService from '../../services/compras/facturas.service.js'
...
const postFactura = async (req, res) => {
  try {
    ...
    if (!id_proveedor || !id_orden_compra || !nro_factura || !codigo_factura || !id_estado)
      return res.status(400).json({ message: 'Faltan datos requeridos' })

    if (!detalles || detalles.length === 0)
      return res.status(400).json({ message: 'Debe incluir al menos un detalle' })

    const factura = await facturasService.postFactura(id_proveedor, id_orden_compra, timbrado, nro_factura, fecha_emision, importe_total, fecha_vencimiento, id_estado, codigo_factura, detalles)
...
const getFacturasPendientesPago = async (req, res) => {
    try {
        const facturas = await facturasService.getFacturasPendientesPago()
        res.status(200).json(facturas)
    } catch (error) {
        res.status(400).json({ message: error.message })
    }
