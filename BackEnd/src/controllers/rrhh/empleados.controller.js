...
const postEmpleado = async (req, res) => {
    try {
        const { ci, conyugue, nro_hijos, hijos_menores, nombre, apellido, ruc, direccion, telefono, correo, tipo_persona, fecha_nacimiento } = req.body

        if (!ci || !nombre)
            return res.status(400).json({ message: 'CI y nombre requeridos' })

        const empleado = await empleadosService.postEmpleado(ci, conyugue, nro_hijos, hijos_menores, nombre, apellido, ruc, direccion, telefono, correo, tipo_persona, fecha_nacimiento)
