...
export default function GestionPersonal({ usuario, onLogout, onNavegar, onGuardado }) {
  ...
  const buildPayload = (cargoActual = null) => {
    const id_cargo = cargoActual?.id_cargo ?? cargosDisponibles.find(c => c.nombre === form.cargo)?.id_cargo
  ...
  const editEmpleado = async () => {
    if (!validar()) return
    try {
      const cargoActual = await crearCargoInexistente()
      const res = await fetchConToken(`${API_BASE}/rrhh/empleados/${id}`, {
        method: "PUT", headers: { "Content-Type": "application/json" },
        body: JSON.stringify(buildPayload(cargoActual)),
      })
      const data = await res.json()
  ...
  const crearCargoInexistente = async () => {
    ...
    if (!existeCargo) {
      ...
      setCargosDisponibles(prev => [...prev, dataC])
      return dataC
    }
    return cargosDisponibles.find(e => e.nombre?.toLowerCase() === form.cargo.toLowerCase())
  }
