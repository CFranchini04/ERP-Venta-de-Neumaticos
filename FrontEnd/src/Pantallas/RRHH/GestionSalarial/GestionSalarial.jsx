...
import {
  fetchCuentas,
  crearAsientoAPI,
} from "../../../Pantallas/Contabilidad/contabilidadHelpers";
  ...
  return (
    ...
    >
      ...
      <div style={styles.itemsLista}>
        {items.length === 0 && (
          <p style={styles.sinItems}>Sin {titulo.toLowerCase()} aun</p>
        )}
        {items.map((item) => (
      ...
      <button
        style={{ ...styles.btnAgregar, color, borderColor }}
        onClick={agregar}
      >
...
export default function GestionSalarial({ usuario, onLogout, onNavegar }) {
    ...
    const cargar = async () => {
      try {
        ...
        setEmpleado({
          ...
          hijosMenores: dataEmp?.hijos_menores ?? 0,
        });
  ...
  const handlePeriodoChange = (nuevo) => {
    if (
      nuevo.fechaInicio &&
      nuevo.fechaFin &&
      nuevo.fechaFin < nuevo.fechaInicio
    )
      return;
  ...
  const hijosMenores = Number(empleado.hijosMenores || 0);
  const subsidioFamiliar = hijosMenores > 0 ? salarioBase * 0.05 * hijosMenores : 0;
  ...
  const totalIngresosSinIps =
    salarioBase + pagoHorasExtras + totalBonificaciones + subsidioFamiliar;
  ...
  const generarAsientoSalarial = async () => {
    try {
      ...
      const cuentaSueldos = buscar("4.1.1.4.1.01");
      const cuentaIPS = buscar("2.1.1.3.02");
      const cuentaSueldosPag = buscar("2.1.1.3.01");
  ...
  const guardarPago = async () => {
    ...
    try {
      ...
      if (pdpModificado || !id_pdp) {
        const resPdp = await fetchConToken(
          `${API_BASE}/rrhh/salarios/procesos`,
          {
            method: "POST",
        ...
        if (!resPdp.ok)
          throw new Error(nuevoPdp.message || "Error al crear proceso");
      ...
      if (!resPago.ok)
        throw new Error(dataPago.message || "Error al guardar pago");
  ...
  const imprimirRecibo = () => {
    ...
    const bonifiHtml = [
      ...
      hijosMenores > 0
        ? `<div class="fila"><span>Subsidio familiar (${hijosMenores} hijo${hijosMenores > 1 ? "s" : ""} × 5%):</span><strong>${formatearGs(subsidioFamiliar)}</strong></div>`
        : "",
      ...bonificaciones.map(
        (b) =>
          `<div class="fila"><span>${b.nombre || "Bonificacion"}:</span><strong>${formatearGs(b.monto)}</strong></div>`
      ),
    ].join("");
    ...
    const deducHtml = [
      ...
      ...deducciones.map(
        (d) =>
          `<div class="fila"><span>${d.nombre || "Deduccion"}:</span><strong>- ${formatearGs(d.monto)}</strong></div>`
      ),
    ].join("");
      ...
      <main style={styles.contenido}>
          ...
          {periodoExpanded && (
            <div style={styles.periodoGeneral}>
              {pdpActual && !pdpModificado && (
                ...
                >
                  Usando proceso mas reciente (ID: {pdpActual.id_pdp}). Cambia
                  las fechas para crear uno nuevo.
                </p>
              )}
              {pdpModificado && (
                <p
        ...
        {nominaExpanded && (
          <div style={styles.seccion}>
              ...
              <div style={styles.baseCard}>
                ...
                {horasExtras > 0 && (
                  <span style={styles.calculado}>
                    = {formatearGs(pagoHorasExtras)} (x1.5)
                  </span>
                )}
              </div>
              <div style={styles.baseCard}>
            ...
            {hijosMenores > 0 && (
              <div style={{ ...styles.ipsInfo, background: "#f0fdf4", border: "1px solid #bbf7d0", color: "#166534" }}>
                <span>
                  Subsidio familiar ({hijosMenores} hijo{hijosMenores > 1 ? "s" : ""} menor{hijosMenores > 1 ? "es" : ""} de 18 años × 5%):
                </span>
                <strong>+ {formatearGs(subsidioFamiliar)}</strong>
              </div>
            )}
        ...
        <div style={styles.botonContainer}>
          {periodoIncompleto && (
            <p style={{ color: "red", margin: 0 }}>
              Debes seleccionar un periodo antes de continuar
            </p>
          )}
          <div style={{ flex: 1 }} />
          <button
            ...
            <div style={styles.modal}>
              ...
              <div style={styles.modalContenido}>
                ...
                {hijosMenores > 0 && (
                  <div style={styles.modalFila}>
                    <span>
                      Subsidio familiar ({hijosMenores} hijo{hijosMenores > 1 ? "s" : ""})
                    </span>
                    <strong style={{ color: "#16a34a" }}>
                      + {formatearGs(subsidioFamiliar)}
                    </strong>
                  </div>
                )}
                {bonificaciones.map((b) => (
