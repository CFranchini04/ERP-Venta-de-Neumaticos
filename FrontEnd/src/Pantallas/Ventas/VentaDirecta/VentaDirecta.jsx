    ...
    try {
      ...
      try {
        ...
        if (resFacturas.ok) {
          ...
          const numeros = arr
            .map((f) => {
              const match = String(f.nro_factura || f.codigo_factura || "")
                .toUpperCase()
                .match(/FC-V-(\d+)/);
      ...
      if (!resPresupuesto.ok) {
        ...
        throw new Error(
          errorData.message || "Error al crear presupuesto base",
        );
      }
      ...
      const idPresupuesto =
        ...
        dataPresupuesto.presupuesto?.id_presupuesto ??
        dataPresupuesto.id;
      console.log("Respuesta presupuesto:", dataPresupuesto, "idPresupuesto:", idPresupuesto);
      if (!idPresupuesto) {
        throw new Error(
          "El backend no devolvió id_presupuesto al crear el presupuesto base",
        );
      }
      ...
      console.log("Respuesta factura:", dataFactura);

      let idFactura =
        dataFactura.id_factura_venta ??
        dataFactura.id_factura ??
        dataFactura.id ??
        dataFactura.factura?.id_factura_venta ??
        dataFactura.factura?.id_factura ??
        dataFactura.factura?.id;

      if (!idFactura && codigoFactura) {
        const resByCodigo = await fetchConToken(
          `${API_BASE}/ventas/facturas/codigo/${codigoFactura}`,
        );
        if (resByCodigo.ok) {
