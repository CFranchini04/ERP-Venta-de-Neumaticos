    ...
    const init = async () => {
      try {
        ...
        if (resOrden.ok) {
          ...
          if (placeholder) {
            setFacturaPlaceholderId(
              placeholder.id_factura_compra ?? placeholder.id
            );
          }
          const detalles =
...
const lineasIniciales = detalles
  .filter((det) => {
    // Excluir productos cuyo detalle de orden fue anulado por nota de crédito
    const esAnulado = (det.estado || '').toLowerCase() === 'anulado'
    return !esAnulado
  })
  .map((det) => {
            ...
            <div style={ms.campo}>
              ...
              <div style={ms.inputReadonly}>
                {proveedores.find(
                  ...
                  : proveedorInfo
                  ? `${proveedorInfo.personas?.nombre ?? ""} ${
                      proveedorInfo.personas?.apellido ?? ""
                    }`.trim()
                  : "Cargando..."}
              </div>
            </div>
