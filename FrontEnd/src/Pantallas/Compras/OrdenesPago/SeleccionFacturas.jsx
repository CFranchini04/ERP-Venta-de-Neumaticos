...
export default function PagoFacturas({ usuario, onNavegar, onLogout }) {
    ...
    const proveedores = Array.from(
        new Map(facturasTodas.map((f) => [f.proveedor, { id: f.proveedor, nombre: f.proveedor }])).values()
    ).filter((p) => {
    ...
    const facturasFiltradas = facturasBruto
        ...
        .sort((a, b) => {
            if (ordenFac === "fechaCreacionDesc") return new Date(b.fecha_creacion) - new Date(a.fecha_creacion);
            if (ordenFac === "fechaCreacionAsc") return new Date(a.fecha_creacion) - new Date(b.fecha_creacion);
            if (ordenFac === "fechaLimiteAsc") return new Date(a.fecha_limite) - new Date(b.fecha_limite);
            return 0;
        });
    ...
    const handleSeleccionarProveedor = (prov) => {
        if (proveedorSeleccionado?.id === prov.id) {
            ...
            setFacturasSeleccionadas([]);
            setListaProvExpanded(false); // contraer al seleccionar
        }
    };
    ...
    const handleProceder = () => {
        navigate("/compras/ordenes-de-pago/pagar-facturas", {
            state: {
                ...
                {!loading && !error && (
                    <>
                        ...
                        {proveedorSeleccionado && (
                            <section style={styles.seccion}>
                                <h2 style={styles.subtitulo}>
                                    2. Seleccione las facturas de <strong>{proveedorSeleccionado.nombre}</strong>
                                </h2>
