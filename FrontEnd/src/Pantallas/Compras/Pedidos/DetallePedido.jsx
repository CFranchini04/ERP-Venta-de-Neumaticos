...
export default function DetallePedido({ usuario, onNavegar, onLogout }) {
  ...
  // ─── Callback al guardar cotización ───────────────────────────────────────
  // Recarga el pedido para reflejar la nueva cotización en la tab
  const handleCotizacionGuardada = () => {
    fetchPedido();             // refresca datos
    setTabActiva("cotizaciones"); // cambia a la tab de cotizaciones
  };
  ...
  // Cotizaciones aplanadas: una fila por detalle de cotización
  const filasCotizaciones = (pedido?.cotizaciones ?? [])
    .flatMap((c) => {
      ...
      return c.detalle.map((d) => ({
        ...
        producto:            d.producto,
        cantidad:            d.cantidad,
        precio_unitario:     Number(d.precio_unitario).toLocaleString("es-PY"),
        subtotal:            Number(d.subtotal).toLocaleString("es-PY"),
        es_mejor_opcion:     d.es_mejor_opcion,
      }));
    })
    .filter((f) => {
      const txt = busquedaCot.toLowerCase();
      return (
        f.producto.toLowerCase().includes(txt) ||
        f.codigo_cotizacion.toLowerCase().includes(txt) ||
        f.proveedor.toLowerCase().includes(txt)
      );
    });
  ...
  const columnsCotizaciones = [
    ...
    { key: "subtotal",          label: "Subtotal"    },
    {
      ...
      label: "Mejor opción",
      render: (item) => (
        <span style={{ fontWeight: item.es_mejor_opcion ? 700 : 400, color: item.es_mejor_opcion ? "#22a000" : "#888" }}>
  ...
  return (
    <div style={styles.pagina}>
          ...
          <div style={styles.infoContainer}>
            ...
            <div>
              ...
              {pedido?.precio_total != null
                ? `${Number(pedido.precio_total).toLocaleString("es-PY")} Gs.`
                : "—"}
            </div>
          </div>
        </div>
          ...
          <div style={styles.cardTabla}>
            ...
            <div style={styles.footer}>
              <h2 style={{ margin: 0, fontFamily: "Lato" }}>
                ...
                <strong>{totalEstimado.toLocaleString("es-PY")} Gs.</strong>
              </h2>
            </div>
          </div>
        )}
          ...
          <div style={styles.cardTabla}>
            <List
              ...
              controls={[
                {
                  type: "search",
                  placeholder: "Buscar por cotización, proveedor o producto...",
                  value: busquedaCot,
      ...
      <CargarCotizacionModal
        ...
        productosPedido={pedido?.detalle ?? []}
        idPedido={id}
        onGuardar={handleCotizacionGuardada}
      />
